...
class Table:
    ...
    def __init__(self, no_of_rows, no_of_cols, images_path, ocr_path, gt_table_path, assigned_category,
        ...
        if (self.assigned_category == 1):
            ...
            self.border_cat = \
                random.choices(self.borders_categories['types'], weights=self.borders_categories['probs'])[0]
    ...
    def define_col_types(self):
        ...
        for i, type in enumerate(
                random.choices(['n', 'w', 'r'], weights=[prob_numbers, prob_words, prob_others], k=self.no_of_cols)):
    ...
    def create(self):
        ...
        if (self.assigned_category == 3):  # if assigned category is 3, then it should have spanned rows or columns
            ...
            local_span_flag = random.choices([True, False], weights=[0.5, 0.5])[
                0]  # randomly choose if to span columns and rows for headers or not
        # local_span_flag=True
        if (local_span_flag):
