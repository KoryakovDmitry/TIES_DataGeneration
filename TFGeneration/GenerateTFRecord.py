...
class GenerateTFRecord:
    def __init__(self,
        ...
        self.num_of_max_vertices = 22500  # number of vertices (maximum number of words in any table)
        self.max_length_of_word = 150  # max possible length of each word
        self.row_min = 2  # minimum number of rows in a table (includes headers)
        self.row_max = 150  # maximum number of rows in a table
        self.col_min = 2  # minimum number of columns in a table
        self.col_max = 150  # maximum number of columns in a table
        ...
        self.max_height = 2339  # max image height
        self.max_width = 1656  # max image width
    ...
    def generate_tf_record(self, im, cellmatrix, rowmatrix, colmatrix, arr, tablecategory, imgindex, output_file_name):
        ...
        feature['global_features'] = tf.train.Feature(float_list=tf.train.FloatList(
            value=np.array([img_height, img_width, no_of_words, tablecategory]).astype(np.float32).flatten()))
        feature['vertex_features'] = tf.train.Feature(
            float_list=tf.train.FloatList(value=vertex_features.astype(np.float32).flatten()))
    ...
    def gen_tf_par(self, ds_path, mode="train", max_threads=4):

        starttime = time.time()
        threads = []
        for threadnum in range(max_threads):
            proc = Process(target=self.gen_own_tf, args=(ds_path, mode))
            proc.start()
            threads.append(proc)
    ...
    def gen_own_tf(self, ds_path, mode="train"):

        dirname = f"/Volumes/Seagate/Initflow/PubTabNetDs/pubtabnetresized/{mode}"
        ...
        for d_img in ds_mode:
            fn = list(d_img.keys())[0]
            data = list(d_img.values())[0]
            img = cv2.imread(osp.join(dirname, fn))
            imgindex = 0
            outfile = f"/Users/dmitry/Initflow/TIES/pubds/{mode}/{fn}".replace(".png", ".tfrecord")
            with tf.io.TFRecordWriter(outfile, options=options) as writer:
                img = np.asarray(img, np.int64)[:, :, 0]
                l = len(data)
                ...
                seq_ex = self.generate_tf_record(img, cellmatrix, rowmatrix, colmatrix, bboxes,
                                                 tablecategory, imgindex, outfile)
    ...
    def generate_tables(self, driver, N_imgs, output_file_name):
        ...
        rc_arr = np.random.uniform(low=row_col_min, high=row_col_max,
                                   size=(N_imgs, 2))  # random row and col selection for N images
        all_table_categories = [0, 0, 0, 0]  # These 4 values will count the number of images for each of the category
        rc_arr[:, 0] = rc_arr[:,
                       0] + 2  # increasing the number of rows by a fix 2. (We can comment out this line. Does not affect much)
        data_arr = []
        exceptioncount = 0
                ...
                while (True):
                    ...
                    try:
                        ...
                        if (self.visualizeimgs):
                            ...
                            f = open(os.path.join(dirname, 'html',
                                                  str(rc_count) + output_file_name.replace('.tfrecord', '.html')), 'w')
                            f.write(html_content)
                            f.close()
                            im.save(os.path.join(dirname, 'img',
                                                 str(rc_count) + output_file_name.replace('.tfrecord', '.png')),
                                    dpi=(600, 600))

                        # driver.quit()
                        # 0/0
                        # l_b = len(bboxes)
                        # same_row_matrix = np.eye(l_b, dtype=np.float64)
                        # same_cell_matrix = np.eye(l_b, dtype=np.float64)
                        # same_col_matrix = np.eye(l_b, dtype=np.float64)

                        data_arr.append(
                            [[same_row_matrix, same_col_matrix, same_cell_matrix, bboxes, [tablecategory]], [im]])
        ...
        while (True):
            ...
            if (data_arr is not None):
                if (len(data_arr) == filesize):
                    with tf.io.TFRecordWriter(os.path.join(self.outtfpath, output_file_name),
                        ...
                        try:
                            for imgindex, subarr in enumerate(data_arr):
                                ...
                                seq_ex = self.generate_tf_record(img, cellmatrix, rowmatrix, colmatrix, bboxes,
                                                                 tablecategory, imgindex, output_file_name)
                                writer.write(seq_ex.SerializeToString())
                            print('\nThread :', threadnum, ' Completed in ', time.time() - starttime, ' ',
                                  output_file_name, 'with len:', (len(data_arr)))
                            print('category 1: ', all_table_categories[0], ', category 2: ', all_table_categories[1],
                                  ', category 3: ', all_table_categories[2], ', category 4: ', all_table_categories[3])
                        except Exception as e:
    ...
    def write_to_tf(self, max_threads):
        ...
        if (not os.path.exists(self.distributionfile)):
            if ((not os.path.exists(self.unlvtablepath)) or (not os.path.exists(self.unlvimagespath)) or (
                    not os.path.exists(self.unlvocrpath))):
